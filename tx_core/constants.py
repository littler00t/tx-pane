"""Module-wide constants for tx.

Holds version strings, on-disk paths (resolved from TX_HOME), the v2 marker
regex, the ANSI strip regex, the set of recognised shell names, and the
DEFAULT_CONFIG dict that seeds ~/.tx/config.toml.

Path constants are captured at import time from os.environ; subprocess
invocations of `tx` therefore see TX_HOME, while in-process re-imports
...
TX_DIR = Path(os.environ.get("TX_HOME") or str(Path.home() / ".tx"))
CONFIG_PATH = TX_DIR / "config.toml"
OFFSETS_PATH = TX_DIR / "offsets.json"
LOGS_DIR = TX_DIR / "logs"
LOCK_PATH = TX_DIR / ".lock"

# Marker protocol (v2): every tx run/exec wraps the user command in a sentinel
...
DEFAULT_CONFIG: dict[str, Any] = {
    ...
    # Log-file rotation (~/.tx/logs/<pane>.log). Rotation triggers when
    # pipe-pane (re)starts and the existing log exceeds max_size_mb. Aged
    # rotated logs are swept by `tx maintain` or lazily on `tx ls`.
    "logs": {
        "max_size_mb": 100,
        "max_age_days": 30,
        "max_keep": 10,
    ...
    # escape hatch: TX_NO_COMPACT=1 short-circuits to identity at every call.
    #
    ...
    # near-identical pitfall), and tx's own marker echoes.
    "compact": {
        "default_mode": "terse",
        ...
        # `tx compact-stats --dedup-would-hit` telemetry, an opt-in
