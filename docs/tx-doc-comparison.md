# tx — comparison with related tools

This doc exists for two audiences:

1. **Humans deciding whether to adopt `tx`** — when is it the right
   tool, and what existing thing should you keep using instead?
2. **Agents that have been asked "can we do this with tx?" and need to
...
| Background commands | recent additions (`run_in_background`+`BashOutput` in Claude Code; equivalent elsewhere) | `tx exec` + `tx wait-run` |
| Output size handling | character truncation | 18 normalizers + 5 layers + recoverable handles |
| Interrupt running command cleanly | varies (often kill the subprocess) | `tx kill-run` (sends C-c; preserves marker) |
| Human can watch in real-time | no | `tmux attach -t tx` |
...
never need to interrupt or watch a running command. Don't add `tx` for
the sake of it.

**When to move to `tx`:** the moment you find yourself doing any of:
...
| Nested-shell exit codes (ssh, sudo -i) | inherited brokenness from prompt-regex | `tx hook-install` reinstalls the hook |
| Output compaction | none | 18 normalizers + 5 layers |
| Elision recoverable | n/a (no elision) | yes via `h-XXX` handles |
| Human-attach to live session | no (PTY is internal) | yes (`tmux attach -t tx`) |
...
prompt templates, etc.). `tx` is intentionally not an MCP server —
shell-out composes with anything, but it isn't one-line drop-in for
an MCP host.

**When `tx` is better:** when the marker-protocol reliability or the
...
It's plausible to wrap `tx` behind an MCP shim; nothing in `tx`'s
design prevents it.
...
`tx` is the opposite premise: give the agent a more opinionated
...
**When to use `tx`:** you're shipping an agent that has to be
...
Terminal-Bench's findings reinforce why `tx` exists: even frontier
...
The bet `tx` makes is that you can buy back a lot of those failures
...
**When to use `tx`:** you want the agent to *drive* — kick off
...
TmuxAI for your interactive shell and `tx` for your CI/agent
...
`tx` and SWE-ReX target different layers:

- SWE-ReX is concerned with **where** the shell runs (sandbox / VM
  isolation, dependency installation, parallel runners).
- `tx` is concerned with **how** the shell is talked to (marker
  protocol, compaction, refuse-on-busy, handoff).

They compose. You could run `tx` inside a SWE-ReX sandbox, or treat
`tx` as the local "outer" shell that drives SWE-ReX as one tool.
There's no overlap in feature set.

**When to use SWE-ReX without `tx`:** you want one-shot,
sandbox-per-task isolation (e.g. SWE-bench-style tasks where each
task is a fresh repo). The sandbox is the right boundary.

**When to use `tx` without SWE-ReX:** you trust the host (your own
...
user" — same thesis `tx` operates on, different surface area.
...
`tx`'s marker hook is the explicit answer to "how do we stop
...
└─ no
   ...
   └─ no
      ...
      ├─ no → keep the generic bash tool. Adding tx is overhead you won't use.
      └─ yes
         │
         Will output frequently exceed ~4KB, or are there `ssh`/`sudo -i`/`docker exec` steps?
         ├─ yes → tx is the strongest fit.
         └─ no  → either tx or an MCP shell server works; pick by ecosystem.
```

If you're a human deciding adoption:

- Set up `tx` in a side directory, point your agent at it for one
  workflow (a deploy, a smoke test), measure: did "`[exit:?]`"
  failures drop? Did average tool-result size drop?
- If both are flat, you don't have the failure modes `tx` solves.
