...
Cross-references:
- Reading errors from `tx`: see "Error message reference" below.
...
The output is what the pane emitted; `tx` just couldn't confirm the
command completed.
...
4. **Pane is dead** — the shell exited. `tx status <pane>` will say
   `dead`. Fix: `tx restart <pane>` (preserves the log).

5. **Marker stripped by an over-eager filter** — unlikely; reproduce
   with `tx run --raw <pane> <cmd>` and grep the log for `TX_END`. If
   the marker is in the log but `tx` didn't see it, that's a bug
   report.

**Avoid** `tx run --no-marker-check`. There is no such flag, and you
...
**What `tx` knows:** there is a tracked run on this pane that has not
...
`tx` thinks it's still running), see the `[exit:?]` symptom above —
the marker probably didn't reach `tx`.

---

## Symptom: `tx run` times out, but the command was supposed to be quick

**Decision tree:**

1. **Is the pane producing output?** `tx dump <pane> --tail 40`.
   - If yes, the command is just slower than `--timeout` (default
     varies; see `tx config`). Bump `--timeout` or use `--wait-for` /
     `--fail-for` to exit on a pattern.
   - If no, something's wrong; continue.

2. **Is the shell prompt visible?** `tx dump <pane> --tail 10` — look
   for the user's `PS1`. If the prompt is there but no marker line:
   the hook is missing or broken. Run `tx hook-install <pane>`.

3. **Is there a TUI on screen?** Symptoms: ANSI escapes,
   `state=tui` in `tx status`. The command opened `less`, `vim`,
   `htop`, etc. Send `q` or `C-c`: `tx key <pane> q` or
   `tx kill-run <pane> <run-id>`.

4. **Is the pane dead?** `tx status` says `dead`. The shell exited
   (often: the command was `exit`, or a SIGHUP killed it).
   `tx restart <pane>`.

5. **None of the above** — the command is genuinely stuck. Capture
   diagnostics, then `tx kill-run`:

   ```
   tx info <pane>                                  > diag.txt
   tx dump <pane> --tail 200 >> diag.txt
   tx kill-run <pane> <run-id>
   ```
...
`tx` should have compacted that. Reasons it might not have:

1. **The config explicitly sets `raw`.** Check `tx config | grep compact` — if
   `default_mode = "raw"`, remove that override or set
   `[compact] default_mode = "terse"` in `~/.tx/config.toml`.
...
4. **The normalizer doesn't exist for that tool yet.** Check
   `tx compact-stats --passthrough` for the top "no normalizer
...
To force compaction on a single call: `tx run --terse <pane> <cmd>`.
...
of `~/.tx/offsets.json` (default: 100 run records kept per pane).
...
1. **The command was backgrounded with `&`** — exit code reported is
   ...
   Use `tx exec` for explicit async with real exit codes.
...
2. **Command was a TUI** — `vim file.txt`, `htop`, etc. The marker
   ...
   `tx key <pane> q` to exit deliberately.

3. **Multi-statement command** — `cd /etc && grep foo bar`. The marker
   only sees the final statement's exit. For accurate per-statement
   tracking, run them as separate `tx run` calls, or wrap in
   `bash -c '...'` if you need atomicity.

4. **stderr was the interesting part** — `tx run` shows stdout+stderr
...
## Symptom: `tx new` hangs or "shell not ready"

**What it means:** `tx new` waits for the hook to be confirmed loaded
in the new pane's shell before returning. If your shell rc files are
slow (e.g. they hit the network), `tx new` may time out.

Fix:

```
tx new <name> --hook-timeout 30     # bump the default
tx new <name> --no-verify           # skip hook verification (use only for known shells)
```
...
gating them on `[ -z "$TX_PANE" ] && …` so `tx`-created panes skip
the slow path. The env var `TX_PANE` is exported by `tx new`.

---

## Symptom: `tx write` refuses with "fish not supported"

`tx write` uses bash/zsh heredoc syntax for the atomic staging step.
Fish doesn't have the equivalent. Workarounds:

1. **Create a bash subshell pane for the deploy** —
   `tx new deploy --shell bash`, use that pane for `tx write`, keep
...
```
tx config --explain "<your command>"
```
...
`confirm_patterns` matches against every `tx run`/`tx exec`/`tx stream`/
`tx sudo` command and asks the local user (the *human*) to acknowledge
...
## Symptom: pane state shows `tui` and `tx run` won't go

`state=tui` means the shell hook hasn't fired and there's been recent
output suggesting a TUI app. `tx run` refuses to send a new command
because typing into `vim`'s normal-mode buffer doesn't do what you
want.

Resolution:

- `tx key <pane> q` — send a `q` (works for `less`, `htop`, `man`).
- `tx key <pane> Escape :q Enter` — vim.
- `tx kill-run <pane> <run-id>` — send `C-c` (works for most TUI
  apps that allow interrupt).
- `tx run --kill-and-run <pane> <new-cmd>` — sends `C-c` then runs.

If `tx` is mis-detecting TUI (e.g. it sees a colored progress bar and
...
`tx maintain` handles rotation, but doesn't run automatically. Add it
to a cron, or run it manually:

```
tx maintain --dry-run     # show what would be rotated
tx maintain
```
...
`tx maintain --force` ignores the per-pane "recently active" check.

---

## Symptom: `tx grep` returns nothing but the pattern is visibly in the log

Two common gotchas:

1. **You used `tx grep <pane> "ERROR"` but the line has `Error:`.** The
...
2. **ANSI escape codes are between the characters of your pattern.**
   `tx grep` searches the compacted form (ANSI-stripped) by default,
   but if you passed `--keep-ansi`, the bytes are interleaved with
   escape sequences. Drop `--keep-ansi` for grep.

---

## Symptom: bytes in `tx output --json --raw` don't match `tx log` output

By design. `tx output` (without `--full`) is *agent-facing* output:
ANSI stripped, markers stripped, redaction applied, optionally
compacted. `tx log` is the raw pipe-pane log: every byte the pane
emitted, including markers, including any redacted bytes (the log is
not rewritten — see `[security]` notes).

For audit: prefer `tx log` (or open the file directly at
`~/.tx/logs/<pane>.log`). For agent-faithful replay: `tx output
...
**Q: Can I use `tx` over SSH (driving panes on a remote machine)?**
A: Indirectly. You run `tx` *on* the machine; you attach to the tmux
session over SSH if you want to watch. Driving a remote `tx` from a
local agent is a "wrap it in a thin RPC" exercise that isn't built in.

**Q: Does `tx` support `nu`, `xonsh`, `elvish`, `oil`?**
...
recoverable from the on-disk log via `tx output --full`. L4 truncation
is lossy in the *response*, recoverable via the handle.

**Q: Can the agent disable `tx`'s safety rails?**
A: No. `[security]` is read from `~/.tx/config.toml` at process
...
include arbitrary writes to `~/.tx`. If you genuinely need the agent
to manage its own config, opt in explicitly with a pane-local
allowlist that permits `tx config set …` — but be aware that's
...
| `[exit:?] (hook missing)` | marker did not arrive in window | `tx hook-install <pane>` |
| `pane <name> in handoff` | tx is paused on this pane | `tx resume <pane>` |
| `pane <name> not found` | id/name doesn't match any pane | `tx ls` to find the right id |
...
| `handle h-XXXX expired` | run rotated out of history | `tx output <pane> <run-id> --full` (use run-id) |
| `pane state is dead` | shell exited | `tx restart <pane>` |
| `tmux server unreachable` | the tmux session disappeared | check `~/.tx/.tmux.sock`; `tx new` will respawn |
