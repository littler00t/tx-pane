# tx-pane — output compaction

Read this when you need to:
- Control how `tx-pane` compacts agent-facing output (`--raw`/`--terse`/`--token-budget`).
- Retrieve content that L4 elided (`--handle h-XXX` + `--range/--grep/--full`).
- Inspect or tune the per-tool normalizer registry.
- Read telemetry (`tx-pane compact-stats`).
...
`[compact] default_mode` in `~/.tx-pane/config.toml`:
...
1. **Recover via the handle** (L4 truncation only) — the elided middle
   ...
   ```
   tx-pane output <pane> --handle h-XXX --range 100-150     # 0-based lines
   tx-pane output <pane> --handle h-XXX --grep "ERROR"      # ±3 context
   tx-pane output <pane> --handle h-XXX --full              # everything, raw
   ```

2. **Re-run with the per-tool normalizer disabled** (L1-L5 still fire):
   ```
   tx-pane run --no-normalize <pane> "iptables -L -n -v"
   ```

3. **Re-run with all compaction off** — last resort, costs tokens:
   ```
   tx-pane run --raw <pane> "smartctl -A /dev/sda"
   ```

4. **Disable per-pane** if a normalizer is consistently wrong for a
   specific tool on a specific pane:
   ```
   tx-pane config <pane> compact.disabled_normalizers=["smartctl-attrs"]
   ```
...
- `TX_PANE_NO_COMPACT=1` — short-circuits at the entry point; byte-identical to `--raw`.
- `TX_PANE_NO_TELEMETRY=1` — disables compact.jsonl writes (compaction still runs).
- `TX_PANE_COMPACT_DEBUG=1` — every compacted response prepends a diagnostic block.
- `TX_PANE_DEBUG=1` — `CompactCtx.verbose = True` → emit footer even on no-savings cases.
...
```
<head lines>
[tx-pane:elided run=r-XXX raw_lines=1200 elided_lines=850 ~3400tok handle=h-1a2b]
[retrieve: tx-pane output <pane> r-XXX --handle h-1a2b --range 30-880   (or --grep PAT / --full)]
<tail lines>
```
...
```
tx-pane output <pane> --handle h-1a2b --full                  # full original, no compaction
tx-pane output <pane> --handle h-1a2b --range 100-150         # 0-based line slice
tx-pane output <pane> --handle h-1a2b --grep "ERROR"          # matches + ±3 context lines
...
is GC'd; `tx-pane output --handle` errors with "handle expired; use `--full`".

Buffer-handles (prefix `b-`) are issued by `tx-pane tail` / `tx-pane dump` /
`tx-pane wait` / `tx-pane grep` / `tx-pane stream` — same lifecycle, separate prefix.
...
```
[tx-pane:compact tier=full layers=L1,L2,L3 in=14000B out=320B saved=98%]
```
...
1. `~/.tx-pane/plugins/*.py`  (user Python plugins)
2. `~/.tx-pane/filters/*.toml` (user TOML filters)
...
**Disable**: `tx-pane run --no-normalize` (per-call) or
...
raises twice consecutively is disabled for the rest of the `tx-pane` process
...
3. On hit within `ttl_seconds` (default 60) **in the same pane**,
   replace the body with `[tx-pane:same-as r-XXX emitted Ns ago handle=h-YYY]`.

The handle is re-used so the agent can still recover the full content.
Never cross-pane. Opt in only after `tx-pane compact-stats --dedup-would-hit`
(P5 telemetry) shows the hit rate is worth the staleness risk.

## Telemetry — `tx-pane compact-stats`

Per-call records at `~/.tx-pane/compact.jsonl` (rotates at 10MB). Privacy:
...
disk. Disable with `TX_PANE_NO_TELEMETRY=1`.

```
tx-pane compact-stats                    # 7-day summary
tx-pane compact-stats --weak             # cmd_heads with <30% savings
tx-pane compact-stats --passthrough      # top tier-3 cmd_heads (missing normalizer)
tx-pane compact-stats --since 2026-05-14T00:00:00Z
tx-pane compact-stats --json
tx-pane compact-stats --forget           # wipe the jsonl + backup
```
...
TOML (line-based filter): drop a file in `~/.tx-pane/filters/<name>.toml`:
...
Python plugin (structural parsing): drop a file in `~/.tx-pane/plugins/<name>.py`:
...
Restart `tx-pane` (or run `from tx_compact.registry import load_registry;
